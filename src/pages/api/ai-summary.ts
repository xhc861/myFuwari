...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const API_URL = import.meta.env.AI_API_URL || 'https://api.suanli.cn/v1/chat/completions';
      ...
      const response = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: '你是一个专业的文章总结助手。请用简洁、清晰的中文总结文章的核心内容，包括主要观点和关键信息。没有字数限制但也不要长篇大论，叙述文章内容，应该针对不同的文章用不同的语气。请使用Markdown格式输出，可以使用标题、列表、加粗等格式来组织内容，使总结更清晰易读。'
            },
            {
              role: 'user',
              content: `请总结以下文章：\n\n标题：${title}\n\n内容：${truncatedContent}`
